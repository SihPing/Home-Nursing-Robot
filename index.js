...
const friends = []  //承接API回傳資料  !!在axios裡console.log有資料，在axious外無
let filterFd = []   //承接符合搜尋keyword的資料 & 承接符合gender的資料
const favoriteList = JSON.parse(localStorage.getItem('favorite')) || [] //localStorage資料存放
...
function cardRender(data) {
  ...
  data.forEach(function(item) {
    if (favoriteList.some(favoriteOne => favoriteOne.id === item.id)) {
      rawHTML += `
    <div class="card mx-3 my-3 col-sm-6 col-md-4 col-lg-3 mb-3" style="width: 14rem;">
  <div id="avatar-box">
  <img src="${item.avatar}" alt="avatar" id="avatar">
  </div>
  <div class ="card-body">
  <h5 class ="card-title">${item.name}${item.surname}</h5>
  
  <div class="d-flex justify-content-center">
  <button class ="btn btn-dark btn-show-info" data-bs-toggle="modal" data-bs-target="#card-modal" data-id ="${item.id}">About Me</button> 
 <button class="btn btn-outline-warning ms-2"><i class="fa-solid fa-heart fa-xl fa-beat" style="--fa-animation-duration: 2s;" data-id="${item.id}"></i></button>
  </div>  
  </div>
</div>`
    } else {
  ...
  <div class="d-flex justify-content-center">
  <button class ="btn btn-dark btn-show-info" data-bs-toggle="modal" data-bs-target="#card-modal" data-id ="${item.id}">About Me</button> 
 <button class="btn btn-outline-dark ms-2"><i class="fa-regular fa-heart fa-xl fa-beat" style="--fa-animation-duration: 2s;" data-id="${item.id}"></i></button>
  </div>  
  </div>
</div>
  `
...
function start() {
  axios
    ...
    .then(function(response) {
      // for(let cardInfo of response.data.results){
...
//功能:加入最愛，將最愛id傳入localStorage
function addFavorite(id) {
  const favoriteOne = friends.find(friend => friend.id === id)
  if (favoriteList.some(friend => friend.id === id)) {
    return alert('此Robot已經在精選清單中！')
  }
  favoriteList.push(favoriteOne)
...
function cardBtnClick(event) {
  if (event.target.matches(".btn-show-info")) {
    ...
    skill()
    //num = num.splice(0,num.length)
  } else if (event.target.matches(".fa-heart")) {
    //功能3-3:啟動函式，將最愛id傳入localStorage 
